	...
	if (class_exists('PDO')) {
		try {
			...
			if ($CFG->db_debug == 'Y') {
				...
				$output = "Database error encountered".$e->getMessage();
...
function db_query($dbh,$query, $debug = false, $die_on_debug = true, $silent = false, $unbuffered = false,$return_resource=false) {
	global $CFG,$db_transaction,$db_retries,$db_timeout;
		...
		try {
			...
			if ($dbh->inTransaction()) {
				for ($i = 1; $i <= $db_retries; $i++) {
					...
					// }
					usleep($db_timeout);
					trigger_error('Transaction failed. Retrying...'.$e->getMessage(),E_USER_WARNING);
				...
				for ($i = 1; $i <= $db_retries; $i++) {
					usleep($db_timeout);
					trigger_error('Single query failed. Retrying...'.$e->getMessage(),E_USER_WARNING);
...
function db_query_array($conn,$query, $key = '', $first_record = false, $unbuffered = false, $val_field = '') {
	global $CFG;
	
	if (class_exists('PDO')) {
		$result = db_query($conn,$query,false,true,false,$unbuffered,true);
		if (!$result)
			return false;
		...
		$result = db_query($conn,$query,false,true,false,$unbuffered);
...
function db_insert($conn,$table, $info, $date = '', $ignore = false, $silent = false, $echo_sql = false, $return_bool = false, $delayed = false) {
...
function db_update($conn,$table, $id, $info, $pk = 'id', $date = '') {
...
function db_delete($conn,$table, $id, $pk = 'id') {
	if (! is_array ( $pk ))
...
function db_start_transaction($dbh) {
